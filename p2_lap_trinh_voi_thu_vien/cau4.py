...
def do_thi_yen_ngua(x,y):
    ...
    mat_yen_ngua_surf= ax.plot_surface(x, y,f,cmap='winter_r',linewidth=0, antialiased=False)
    ax.set_zlim(-60,40)
...
def do_thi_hyperbolic_1tang(x,y):
    ...
    fig, ax = plt.subplots(subplot_kw={"projection":"3d"})
    hyperbolic_surf= ax.plot_surface(x, y,f1,cmap= 'bwr',linewidth=0, antialiased=False)
    hyperbolic_surf= ax.plot_surface(x, y,f2,cmap= 'bwr',linewidth=0, antialiased=False)
    ax.set_zlim(-8,8)
...
def do_thi_matcau(x,y):
    ...
    fig, ax = plt.subplots(subplot_kw={"projection":"3d"})
    mat_cau_surf= ax.plot_surface(x, y,f,cmap= 'viridis_r',linewidth=0, antialiased=False)
    mat_cau_surf= ax.plot_surface(x, y,f2,cmap= 'viridis_r',linewidth=0, antialiased=False)
    ax.set_zlim(0,7)
